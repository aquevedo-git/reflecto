from datetime import timedelta

from infrastructure.persistence.session_repository import SessionRepository
from infrastructure.providers import SystemTimeProvider, UUIDProvider

SESSION_ID = "d7255ec0-7bad-4ef9-b83f-3a11edae380e"


time_provider = SystemTimeProvider()
id_provider = UUIDProvider()
repo = SessionRepository(time_provider=time_provider, id_provider=id_provider)

for i in range(3):
    day = (time_provider.now() - timedelta(days=i)).isoformat()[:10]

    repo.append_event({
        "id": id_provider.new_id(),
        "session_id": "demo",
        "timestamp": day + "T09:00:00",
        "type": "presence",
        "payload": {"state": "focused"},
        "source": "test"
    })

    repo.append_event({
        "id": id_provider.new_id(),
        "session_id": "demo",
        "timestamp": day + "T12:00:00",
        "type": "skills",
        "payload": {"focus": 95},
        "source": "test"
    })

print("Seeded pattern events")
